...
IF(MONTH(CURRENT_TIMESTAMP))<10 BEGIN
	...
	VALUES(@fecha, @kwh, @limpieza, @parking)
END
	ELSE BEGIN
...
WHILE(@iterar<50) BEGIN
	...
	IF NOT EXISTS(SELECT matricula FROM dbo.datosCoches WHERE matricula=dbo.generarMatricula()) BEGIN
...
AS BEGIN
    ...
    IF EXISTS(SELECT matricula FROM dbo.movEntrada WHERE matricula=@matricula AND vehiculoEnParking=1) BEGIN
        ...
        --ESTÁ ABONADO
        IF EXISTS(SELECT dbo.AbonoClientes.matricula FROM dbo.AbonoClientes INNER JOIN dbo.movEntrada ON dbo.AbonoClientes.matricula=dbo.movEntrada.matricula AND vehiculoEnParking=1 AND dbo.AbonoClientes.anioMes=@abonoFecha)BEGIN
        ...
        END
            --NO ESTÁ ABONADO
            ELSE IF NOT EXISTS(SELECT dbo.AbonoClientes.matricula FROM dbo.AbonoClientes INNER JOIN dbo.movEntrada ON dbo.AbonoClientes.matricula=dbo.movEntrada.matricula AND vehiculoEnParking=1 AND dbo.AbonoClientes.anioMes=@abonoFecha) BEGIN
...
AS BEGIN
	--DATOS COMPROBAR--
	DECLARE @fechaPago DATETIME=(SELECT TOP 1 fechaPago FROM dbo.movSalida WHERE matricula=@matricula AND fechaSalida IS NULL ORDER BY fechaPago DESC)
	DECLARE @fechaActual DATETIME=(CURRENT_TIMESTAMP)
	DECLARE @diferencia INT=CONVERT(INT, (DATEDIFF(MINUTE, @fechaPago, @fechaActual)))
	DECLARE @abonoFecha VARCHAR(MAX)=(SELECT anioMes FROM dbo.AbonoClientes WHERE matricula=@matricula)
	-------------------
		...
		IF(@diferencia)<15 BEGIN
			...
			WHERE matricula=@matricula AND fechaSalida IS NULL
		END
			...
			ELSE BEGIN
				...
				END
					...
					ELSE IF NOT EXISTS(SELECT matricula FROM dbo.AbonoClientes WHERE matricula=@matricula) BEGIN
...
AS BEGIN
    ...
    DECLARE @servicio VARCHAR(MAX)=(SELECT TOP 1 servicioContratado FROM dbo.movEntrada INNER JOIN inserted ON dbo.movEntrada.matricula=inserted.matricula AND dbo.movEntrada.vehiculoEnParking=1 AND inserted.matricula=@matricula)
    --------------------
    --COMPROBAMOS QUE EL VEHÍCULO EXISTS
    --EL VEHÍCULO ESTÁ EN EL PARKING
    IF(SELECT TOP 1 costeServicios FROM inserted WHERE matricula=@matricula)>0 BEGIN
        ...
        IF EXISTS(SELECT enviadoATrafico FROM dbo.movSalida WHERE matricula=@matricula AND enviadoATrafico=0) BEGIN
            ...
            IF (UPPER(@servicio))IN ('P', 'L') BEGIN
                ...
                WHERE matriculaBarreraEntrada=@matricula AND subirBarreraSalida=0 AND matriculaBarreraSalida='???'
                WAITFOR DELAY '00:00:03'
				UPDATE dbo.movBarrera
                SET subirBarreraSalida=0, subirBarreraEntrada=0, matriculaBarreraSalida=@matricula
                WHERE matriculaBarreraEntrada=@matricula AND subirBarreraEntrada=1 AND subirBarreraSalida=1
            END
                --RECARGA Y TODO
...
TRUNCATE TABLE dbo.datosCoches
TRUNCATE TABLE dbo.AbonoClientes
*/
/*************/
/*SIMULACION */
/*************/
...
DECLARE @matricula VARCHAR(MAX)=(SELECT matricula FROM dbo.datosCoches WHERE id=@contador)
